...
// console.log("uniCloud.getCurrentUserInfo(): " + JSON.stringify(uniCloud.getCurrentUserInfo()));
// let 剩余过期时间=uniCloud.getCurrentUserInfo().tokenExpired-Date.now()-1000*60*60*24*3
let 剩余过期时间=uniCloud.getCurrentUserInfo().tokenExpired-Date.now()
console.log("剩余过期时间222: " + JSON.stringify(剩余过期时间/(1000 * 60 * 60)));
if (剩余过期时间 <= 0) {
	// 清空后，后面的hasLogin判断就会为空了
	hostUserInfo={}
}
...
const data = {
	...
	// hasLogin: Object.keys(hostUserInfo).length != 0 && 剩余过期时间>0
}
	...
	async updateUserInfo(data = false) {
		if (data) {
			...
			try {
				...
				console.log('updateUserInfo 返回结果：',res.result.data[0]);
				this.setUserInfo({
					...res.result.data[0],
